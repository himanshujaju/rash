...
# Unit tests for FIFOCache
# Run `./fifo_cache_test.rb` to run these tests
...
def AddWithUpdatedAccessTimeTest()
  cache_size = 7
  range = 0...cache_size
  cache = FIFOCache.new(cache_size)
  range.each do |i|
  ...
  cache.add(target_key, target_value, true)
...
def AddWithoutUpdatedAccessTimeTest()
  cache_size = 7
  range = 0...cache_size
  cache = FIFOCache.new(cache_size)
  range.each do |i|
  ...
  target_key = "key_0"
  target_value = "new_target_value"
  cache.add(target_key, target_value, false)
  assert(cache.find(target_key) == target_value)
  assert(cache.size() == cache_size)
...
def RunAllTests()
  ...
  AddWithoutUpdatedAccessTimeTest()
  AddTooManyItemsTest()
